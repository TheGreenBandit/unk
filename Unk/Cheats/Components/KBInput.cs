...
namespace Unk.Cheats.Components
{
    internal class KBInput : MonoBehaviour
    {
        ...
        {
            get => forward == Vector3.zero ? forward = SemiFunc.MainCamera().transform.forward : forward;
            set => forward = value;
        }

        public Vector3 Right
        {
            get => right == Vector3.zero ? right = SemiFunc.MainCamera().transform.right : right;
            set => right = value;
        }

        public Vector3 Up
        {
            get => up == Vector3.zero ? up = SemiFunc.MainCamera().transform.up : up;
            set => up = value;
        }
